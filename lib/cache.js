...
module.exports = class Cache {
  ...
  async refreshCache() {
    ...
    this.latest.pub_date = formatRFC3339(release.published_at);
    ...
    for (const asset of release.assets) {
      ...
      const signature = release.assets
        ? await getSignature(name, release.assets, token)
        : "";

      const localFile = await saveFile(url, `./files`, name, token);
